...
extratitleSubfield :: Char
extratitleSubfield = 'b'
...
lookupExtraTitle :: MarcRecordRaw -> Maybe Title
lookupExtraTitle = lookupValue titleTag extratitleSubfield

lookupAuthor :: MarcRecordRaw -> Maybe Author
lookupAuthor = lookupValue authorTag titleSubfield

marcToPairs :: B.ByteString -> [(Maybe Title, Maybe Author)]
marcToPairs marcStream = zip titles authors
  where records = allRecords marcStream
        titles = map (\ record ->
                              let title = (lookupTitle record)
                                  extraTitle = (lookupExtraTitle record)
                                  fullTitle = if (isJust title) && (isJust extraTitle)
                                              then title <> (Just ", ") <> extraTitle
                                              else Nothing
                              in
                                fullTitle)
                          records
